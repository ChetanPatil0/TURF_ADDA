          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <ActionTile icon={RiUserAddLine} label="Add Admin" desc="Create new admin" onClick={go('/add-admin')} accent />
                <ActionTile icon={RiShieldCheckLine} label="Pending Turfs" desc="Verify new venues" onClick={go('/turfs/pending')} />
                <ActionTile icon={RiGroupLine} label="Manage Users" desc="View & control users" onClick={go('/users')} />
                <ActionTile icon={RiBarChartBoxLine} label="Platform Reports" desc="Analytics & logs" onClick={go('/reports')} />
              </div>
            </section>

            <Card title="Pending Turf Verifications" badge={stats.pendingTurfs} onViewAll={go('/turfs/pending')}>
                ...
                <div className="divide-y divide-[var(--color-divider)]">
                  {pendingTurfs.slice(0, 5).map((turf) => (
                    <PendingTurfRow
                      key={turf.id}
                      turf={turf}
                      onView={() => navigate(`/turf/${turf.id}/verify`)}
                    />
                  ))}
                </div>
              )}
