...
import { getTurfById, updateTurf } from '../../api/turfApi';
import { useMessageModal } from '../../context/MessageModalContext';
import Loader from '../../components/common/Loader';
import { BASE_URL_MEDIA, sizeOptions, slotDurations, surfaceOptions, sportOptions, amenitiesList } from '../../const';
...
const validationSchema = Yup.object({
  ...
  passbookImage: Yup.mixed().nullable(),
});
...
export default function EditTurf() {
  ...
  useEffect(() => {
    const fetchTurf = async () => {
      try {
        ...
        const turfData = res?.data?.turf || res?.turf || null;
        if (!turfData) throw new Error('Turf not found');

        const loc = turfData.location || {};
        const contact = turfData.contact || {};
        const bank = turfData.bankDetails || {};
        ...
        setInitialValues({
          ...
          openingTime: turfData.openingTime || '07:00',
          closingTime: turfData.closingTime || '23:00',
          slotDurationMinutes: turfData.slotDurationMinutes || 60,
          pricePerSlot: turfData.pricePerSlot || '',
          sports: turfData.sports || [],
          surfaceType: turfData.surfaceType || 'artificial_turf',
          size: turfData.size || '5-a-side',
        ...
        if (turfData.coverImage) setCoverPreview(`${BASE_URL_MEDIA}${turfData.coverImage}`);
        if (turfData.images?.length) setImagesPreviews(turfData.images.map(img => `${BASE_URL_MEDIA}${img}`));
        if (turfData.videos?.length) setVideosPreviews(turfData.videos.map(vid => `${BASE_URL_MEDIA}${vid}`));
        if (bank.passbookImage) setPassbookPreview(`${BASE_URL_MEDIA}${bank.passbookImage}`);
      } catch (err) {
        showMessage({
          type: 'error',
          title: 'Error',
          message: 'Failed to load turf details. Please try again.',
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      // Fixed: append each sport individually (no JSON.stringify)
      (values.sports || []).forEach(sport => formData.append('sports', sport));

      // Fixed: append each amenity individually (no JSON.stringify)
      (values.amenities || []).forEach(amenity => formData.append('amenities', amenity));
      ...
      if (values.videos && values.videos.length > 0) {
        Array.from(values.videos).forEach(file => formData.append('videos', file));
      }

      await updateTurf(id, formData);
      ...
      showMessage({
        type: 'error',
        title: 'Error',
        message: err?.response?.data?.message || 'Failed to update turf. Please try again.',
      });
    } finally {
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-[var(--color-text-secondary)]">
                        Pin your turf on the map – city & state will be selected manually
                      </p>
                    </div>
                ...
                {step === 4 && (
                  <div className="space-y-6">
                    ...
                    <div className="space-y-6">
                      <div>
                        <label className="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
                          Cover Image <span className="text-[var(--color-error)]">*</span>
                        </label>
                          ...
                          {coverPreview ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-12 px-4">
                              ...
                              <p className="text-sm font-medium text-[var(--color-text-primary)] mb-1">
                                Click to upload or replace cover image
                              </p>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
                          Gallery Images <span className="text-[var(--color-error)]">*</span>
                        </label>
                        <p className="text-xs text-[var(--color-text-secondary)] mb-3">
                          Upload at least 3 photos showing different angles
                        </p>
                        <div
                        ...
                        >
                          <div className="flex flex-col items-center">
                            <HiOutlinePhotograph className="text-[var(--color-text-secondary)] mb-3" size={48} />
                            <p className="text-sm font-medium text-[var(--color-text-primary)] mb-1">
                              Click to upload or replace gallery images
                            </p>
                            <p className="text-xs text-[var(--color-text-secondary)]">
                        ...
                        <p className="text-xs text-[var(--color-text-secondary)] mt-2">
                          {values.images ? `${values.images.length} image(s) selected` : 'No new images selected (existing will be kept)'}
                        </p>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-[var(--color-text-primary)] mb-2">
                          Videos <span className="text-[var(--color-error)]">*</span>
                        </label>
                        <p className="text-xs text-[var(--color-text-secondary)] mb-3">
                          Upload at least 1 video showing the turf
                        </p>
                        <div
                          className="border-2 border-dashed border-[var(--color-divider)] rounded-xl p-6 hover:border-[var(--color-primary)] transition-all cursor-pointer bg-white"
                        ...
                        >
                          <div className="flex flex-col items-center">
                            ...
                            <p className="text-sm font-medium text-[var(--color-text-primary)] mb-1">
                              Click to upload or replace videos
                            </p>
                            <p className="text-xs text-[var(--color-text-secondary)]">
                              MP4, MOV recommended
                            </p>
                        ...
                        <p className="text-xs text-[var(--color-text-secondary)] mt-2">
                          {values.videos ? `${values.videos.length} video(s) selected` : 'No new videos selected (existing will be kept)'}
                        </p>
                        ...
                        >
                          {passbookPreview ? (
                            <div className="p-4 text-center">
                              {passbookPreview.includes('.pdf') ? (
                                <p className="text-sm text-[var(--color-text-secondary)]">
                                  Current PDF: {passbookPreview.split('/').pop()}
                                </p>
                              ) : (
                                <img
                                  src={passbookPreview}
                            ...
                            <div className="flex flex-col items-center justify-center py-12 px-4">
                              <HiOutlinePhotograph className="text-[var(--color-text-secondary)] mb-3" size={48} />
                              <p className="text-sm font-medium text-[var(--color-text-primary)] mb-1">
                                Click to upload or replace passbook / cheque
                              </p>
                              <p className="text-xs text-[var(--color-text-secondary)]">
                                JPG, PNG, PDF • max 5MB
                              </p>
                    ...
                    >
                      {isSubmitting ? (
                        ...
                        <>
                          ...
                          <span className="text-sm">Update Turf</span>
                        </>
                      )}
                    </button>
                  )}
