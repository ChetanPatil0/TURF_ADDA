  ...
  try {
    ...
    if (lat && lng) {
      ...
      if (!isNaN(parsedLat) && !isNaN(parsedLng)) {
        ...
        useGeoDistance = true;
        locationType = 'geo';
        locationMessage = 'Near your current location';
      }
    }
    ...
    if (!useGeoDistance) {
      if (user.city && user.city.trim()) {
        cityFilter = user.city.trim();
        fallbackMode = 'city';
        locationType = 'city';
        locationMessage = `Near ${user.city}`;

        if (user.area && user.area.trim()) {
          areaFilter = user.area.trim();
          locationMessage = `Near ${user.area}, ${user.city}`;
        }
      } else {
        fallbackMode = 'popular';
        locationType = 'popular';
        locationMessage = 'Popular turfs across cities';
      }
    }
    ...
    upcomingBookings = upcomingBookings.filter(booking => booking.turf != null);

    const formattedBookings = upcomingBookings.map((booking) => ({
      bookingId: booking._id.toString(),
      turfName: booking.turf?.name || 'Turf (removed)',
      turfId: booking.turf?._id?.toString() || null,
      turfPublicId: booking.turf?.id || null,
    ...
    if (useGeoDistance) {
      ...
      let turfs = await Turf.find({
        ...
        .select(
          'name slug coverImage location.area location.city ' +
    ...
    } else if (fallbackMode === 'city') {
      const query = {
        isActive: true,
        isVerified: true,
        status: 'active',
        'location.city': { $regex: new RegExp(`^${cityFilter}$`, 'i') },
      };

      if (areaFilter) {
        query['location.area'] = { $regex: new RegExp(`^${areaFilter}$`, 'i') };
      }
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        location: {
          type: locationType,
          hasValidLocation: useGeoDistance,
